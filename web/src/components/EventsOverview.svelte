...
<section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
  ...
  <div class="flex flex-wrap items-end gap-4 mb-6">
    <!-- View Toggle -->
    <div>
      <span class="block text-sm font-medium text-haw-blau mb-1">&nbsp;</span>
      <div class="inline-flex rounded border border-haw-blau/20 overflow-hidden">
        <button
          on:click={() => (viewMode = 'list')}
          class="px-3 py-1.5 text-sm transition-colors cursor-pointer
            {viewMode === 'list' ? 'bg-haw-blau text-white' : 'bg-white text-haw-blau hover:bg-haw-blau/10'}"
        >
          {$t('events.view.list')}
        </button>
        <button
          ...
          class="px-3 py-1.5 text-sm transition-colors cursor-pointer
            {viewMode === 'calendar' ? 'bg-haw-blau text-white' : 'bg-white text-haw-blau hover:bg-haw-blau/10'}"
        >
        ...
        {#each upcoming as event (event.id)}
          <div class="bg-white border border-haw-blau/10 rounded-lg shadow-sm overflow-hidden">
            <div class="border-t-4 border-haw-blau p-6">
              ...
              <h3 class="font-serif text-xl font-bold text-haw-blau mb-1">{eventTitle(event)}</h3>
              {#if eventOrganizer(event)}
                <p class="text-xs text-haw-blau/60 mb-2">{$t('events.label.organizer')}: {eventOrganizer(event)}</p>
              {/if}
